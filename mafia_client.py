...
class MafiaClient:
    def __init__(self, address):
        ...
        threading.Thread(target=self.voice_chat_client.send_data_to_server, daemon=True).start()
    ...
    async def run(self) -> None:
        ...
        resp = self.conn.ConnectClient(pb2.ConnectRequest(login=name, voiceChatServerAddr=addr, voiceChatServerPort=port))
        ...
        while self.running:
            # daytime
            print(f'Day {self.round} started, possible actions: vote, end, exit, mute')
            ...
            while self.running and self.playing and self.is_day:
                ...
                if action == 'vote':
                    ...
                    player_name = input('Which player do you want to vote for? ')
                ...
                if pb2.Result.Name(resp.result) != "OK":
                    print(f"Error: {pb2.Result.Name(resp.result)} {resp.message}")
    ...
    def __listen_for_messages(self):
        for message in self.conn.GetMessages(pb2.Empty()):
            print(f'Received message: {pb2.MessageType.Name(message.type)} - {message.message}')
...
if __name__ == '__main__':
    logging.basicConfig()
    address = input('Enter mafia server address (e.g. localhost:50051): ')
