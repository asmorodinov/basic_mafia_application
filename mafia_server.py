...
class GameState:
    def __init__(self, players, seed=None):
        ...
        logging.info(f'Game is initialised, mafias: {self.mafias}, commissars: {self.commissars}, civilians: {self.civilians}')
...
class MafiaServer(pb2_grpc.MafiaServerServicer):
    def __init__(self):
        ...
        threading.Thread(target=self.voice_chat_server.accept_connections, daemon=True).start()
    ...
    def ConnectClient(self, request: pb2.ConnectRequest, context) -> pb2.Response:
        logging.info(f'ConnectClient {{login="{request.login}"}} - {context.peer()}')
    ...
    def __connect_to_voice_chat_room(self, login, room):
        ...
        self.voice_chat_server.room_to_logins[room] = self.voice_chat_server.room_to_logins.get(room, set()) | {login}
    ...
    def ConnectToSpecificRoom(self, request: pb2.ConnectToRoomRequest, context) -> pb2.Response:
        logging.info(f'ConnectToSpecificRoom {{room="{request.room}"}} - {context.peer()}')
        ...
        self.messages.append({'type': pb2.MessageType.Info, 'message': f'Welcome to room "{room}", {login}!', 'to': address})
        self.messages.append({'type': pb2.MessageType.Info, 'message': f'Players in the room: {self.room_to_logins[room]}.', 'to': address})

        # send info to other members of the room
        for other in self.room_to_logins[room]:
            if other != login:
                other_address = self.login_to_address[other]
                self.messages.append({'type': pb2.MessageType.Info, 'message': f'{login} joined room', 'to': other_address})
        ...
        if len(self.room_to_logins[room]) == self.max_room_size:
            ...
            for player, role in self.room_to_state[room].roles.items():
                ...
                self.messages.append({'type': pb2.MessageType.Info, 'message': f'Game is starting!', 'to': addr})
                self.messages.append({'type': pb2.MessageType.Info, 'message': f'Your role is - {role}', 'to': addr})
                self.messages.append({'type': pb2.MessageType.AssignedRole, 'message': f'{role}', 'to': addr})
    ...
    def __end_day_check(self, room, state):
        ...
        if voted_for:
            ...
            for player in state.players:
                if player in self.login_to_address:
                    ...
                    self.messages.append({'type': pb2.MessageType.Info, 'message': f'Players voted for {voted_for}', 'to': address})
        ...
        for player in state.players:
            if player in self.login_to_address:
                address = self.login_to_address[player]
                self.messages.append({'type': pb2.MessageType.DayEnded, 'message': f'Day ended', 'to': address})
    ...
    def __end_night_check(self, room, state):
        ...
        if voted_for:
            ...
            for player in state.players:
                if player in self.login_to_address:
                    address = self.login_to_address[player]
                    self.messages.append({'type': pb2.MessageType.Info, 'message': f'Mafia killed {voted_for}', 'to': address})
        ...
        for player in state.players:
            if player in self.login_to_address:
                address = self.login_to_address[player]
                self.messages.append({'type': pb2.MessageType.NightEnded, 'message': f'Night ended', 'to': address})
    ...
    def VoteToKill(self, request: pb2.Player, context) -> pb2.Response:
        ...
        success, err, login, room, state = self.__can_perform_action_in_game(address)
        if not success:
            return err
        ...
        if is_ended:
            ...
            if room in self.room_to_logins:
                for player in self.room_to_logins[room]:
                    address = self.login_to_address[player]
                    self.messages.append({'type': pb2.MessageType.GameEnded, 'message': f'Game has ended! {result} won.', 'to': address})
    ...
    def __disconnect(self, addr):
        ...
        if login is not None and room in self.room_to_logins:
            for other in self.room_to_logins[room]:
                if other != login:
                    other_address = self.login_to_address[other]
                    self.messages.append({'type': pb2.MessageType.Info, 'message': f'{login} left room', 'to': other_address})
        ...
        if login is not None and room in self.room_to_state:
            ...
            self.messages.append({'type': pb2.MessageType.YouWereKilled, 'message': f'You were killed', 'to': addr})
...
if __name__ == '__main__':
    ...
    port = input("Select port to run server on (e.g. 50051): ")
